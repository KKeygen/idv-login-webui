...
<template>
  <section class="content-page cloud-page">
    ...
    <ModalShell :open="wizardOpen" title="云同步配置向导" @close="wizardOpen = false">
      ...
      <div v-if="wizardStep === 1" class="wizard-step"><ShieldCheck :size="48" /><h3>先了解数据如何保存</h3><div class="wizard-explanation"><p>{{ policy?.storage || '云端仅保存加密后的账号记录，主密钥不会上传。' }}</p><p>主密钥是访问、解密、修改和删除云端记录的唯一凭证。工具和云端都无法在丢失后帮你恢复它。</p></div><label class="consent"><input v-model="form.consent_ack" type="checkbox" /><span>我已阅读并理解 <a :href="CLOUD_SYNC_POLICY_URL" target="_blank" rel="noopener noreferrer" class="policy-link" @click.stop.prevent="openExternal(CLOUD_SYNC_POLICY_URL)">云同步详细说明和隐私政策</a></span></label><label class="field"><span>使用场景</span><select v-model="wizardMode"><option value="import_existing">导入已有同步</option><option value="create_new">创建新的同步</option></select><small>导入时会校验云端记录；创建时将在首次上传后生成新记录。</small></label></div>
      <div v-else-if="wizardStep === 2" class="wizard-step"><KeyRound :size="48" /><h3>设置主密钥</h3><p class="wizard-lead">{{ wizardMode === 'import_existing' ? '输入创建该云端记录时使用的主密钥，下一步会检查它是否对应有效数据。' : '生成或输入一个新主密钥，并立即在安全位置保存副本。' }}</p><div class="inline"><input v-model="form.master_key" placeholder="粘贴或生成主密钥" /><button class="primary" @click="generateKey">生成</button></div><p>密钥强度：{{ strength.label }}（至少 12 位且包含三类字符）</p><div class="inline"><button class="ghost" @click="downloadKey">保存为文本</button><label class="ghost file-button">从文本导入<input type="file" accept=".txt,text/plain" @change="importKey" /></label></div><p v-if="probeResult" class="muted">{{ probeResult.exists ? `找到约 ${probeResult.remote_channels_count || 0} 个云端账号` : probeResult.message }}</p></div>
      <div v-else-if="wizardStep === 3" class="wizard-step"><Cloud :size="48" /><h3>同步选项</h3><p class="wizard-lead">账号范围只限制上传内容。双向同步会先下载并合并同 UUID 的最新记录，再上传合并结果。</p><label class="field"><span>账号范围</span><select v-model="form.scope_type"><option value="all">所有账号</option><option value="current_game">当前游戏</option><option value="selected">指定账号</option></select></label><label class="field"><span>同步方向</span><select v-model="form.sync_direction"><option value="pull">云端 → 本地</option><option value="push">本地 → 云端</option><option value="bidirectional">双向同步</option></select></label><label class="setting-row"><span><strong>自动同步</strong><small>需要在本地记住主密钥；启动时同步，账号变化 5 秒后上传。</small></span><input v-model="form.auto_sync" type="checkbox" /></label></div>
