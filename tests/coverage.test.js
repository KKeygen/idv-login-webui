...
describe('legacy API surface', () => {
  ...
  it('keeps game settings scoped internally while using game language in the UI', () => {
    ...
    expect(settings).toContain('distribution_id: distributionId.value')
    expect(settings).toContain('installation_id: installationId.value')
  ...
  it('matches the Starward selector and shared start-button structure', () => {
    ...
    expect(rail).toContain(':class="{ active: isActive(item) }"')
  ...
  it('keeps navigation animated while loading game-specific data on demand', () => {
    ...
    expect(app).toContain('name="page-change"')
    ...
    expect(store).not.toContain('warmLauncherImages({ catalog')
    ...
    expect(styles).toContain('@keyframes game-scene-in')
  })
